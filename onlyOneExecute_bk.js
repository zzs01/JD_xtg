...
async function start() {
    ...
    function startTime(){
        ...
        const today_zero_time = new Date(new Date().toLocaleDateString());
        start_run.setTime(start_run.getTime() + 3600 * 1000 * 24 * 1);
		let wait_time = []
        wait_time[0] = today - today_zero_time; //过零点时间
		wait_time[1] = start_run - today; //到零点时间
		return wait_time
    }
    ...
    let waiting_time2 = 0;
    if (Secrets.LOCATION) {	
    	waiting_time1 = startTime()[0]
        waiting_time2 = startTime()[1]
    }
    if (waiting_time1 > 1200000 and waiting_time2 > 300000) {
        console.log("现在没有京豆了零点之前五分钟内或零点后二十分钟内再执行吧");
        return
    }
    if (waiting_time2 > 300000) {
        console.log("需要等待" + waiting_time2 / 1000 + "s" + "，等待时间过长，零点之前五分钟内再执行吧");
        return
    }
    else{
        console.log("脚本等待" + waiting_time2 / 1000 + "s");
        sleep(waiting_time2);
    }
